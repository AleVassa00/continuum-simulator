...
func connect(
	...
	brokerURL, err := url.Parse(brokerAddress)
	if err != nil {
		return nil, fmt.Errorf(
			"parse MQTT broker URL %q: %w",
			brokerAddress,
			err,
		)
	}
	...
	clientConfig := paho.ClientConfig{
		...
		OnClientError: func(err error) {
			log.Printf(
				"component=mqtt client_id=%s level=error error=%q",
				clientID,
				err,
			)
		},
	...
	if onPublishReceived != nil {
		clientConfig.OnPublishReceived =
			[]func(paho.PublishReceived) (bool, error){
				onPublishReceived,
			}
	}

	connection, err := autopaho.NewConnection(
		ctx,
		autopaho.ClientConfig{
			...
			KeepAlive: uint16(
				cfg.KeepAlive.Duration / time.Second,
			),

			CleanStartOnInitialConnection: true,
			SessionExpiryInterval:         0,

			ReconnectBackoff: autopaho.NewConstantBackoff(
				time.Second,
			),
			...
			OnConnectError: func(err error) {
				log.Printf(
					"component=mqtt client_id=%s broker=%s level=warning connect_error=%q",
					clientID,
					brokerAddress,
					err,
				)
