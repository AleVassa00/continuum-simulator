...
func TestKafkaPartitionPipeline(t *testing.T) {
	...
	for _, count := range []int{1, 3, model.DefaultSourcePartitionCount, 8} {
		for _, edgeCount := range []int{13, 1} {
			var baseline []model.GlobalAggregate
			for _, workers := range []int{1, 2, 4, 6} {
				t.Run(fmt.Sprintf("P%d/edges%d/W%d", count, edgeCount, workers), func(t *testing.T) {
					...
					err = conn.CreateTopics(kafka.TopicConfig{Topic: input, NumPartitions: count, ReplicationFactor: 1}, kafka.TopicConfig{Topic: output, NumPartitions: 1, ReplicationFactor: 1})
					conn.Close()
					if err != nil {
						t.Fatal(err)
					}
					...
					pollKafka(t, ctx, func() bool {
						r, err := client.OffsetFetch(ctx, &kafka.OffsetFetchRequest{GroupID: group, Topics: map[string][]int{input: partitionIDs}})
						if err != nil || r.Error != nil || len(r.Topics[input]) != count {
							return false
						}
