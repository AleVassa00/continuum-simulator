...
func TestKafkaCompletionFollowsAcknowledgedPartitionData(t *testing.T) {
	...
	for _, count := range []int{2, model.DefaultSourcePartitionCount, 8} {
		t.Run(fmt.Sprintf("P%d", count), func(t *testing.T) {
			// Each experiment is a separate process. Do not carry metadata cached
			// for previous test topics into this run's producers.
			previousTransport := kafka.DefaultTransport
			transport := &kafka.Transport{}
			kafka.DefaultTransport = transport
			defer func() { transport.CloseIdleConnections(); kafka.DefaultTransport = previousTransport }()
			...
			err = conn.CreateTopics(kafka.TopicConfig{Topic: topic, NumPartitions: count, ReplicationFactor: 1})
			conn.Close()
			if err != nil {
				t.Fatal(err)
			}
